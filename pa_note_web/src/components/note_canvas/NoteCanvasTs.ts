...
// import {N} from "@/logic/note/note";
import {search} from "@/logic/note/search";
...
export module NoteCanvasTs{
    ...
    export class ChunkHelper {
        ...
        calc_chunk_pos_(x:number, y:number) {
            let cx = 0, cy = 0;
            cx = Math.floor(x / 300);
            cy = Math.floor(y / 400);
            return {cx , cy}
        ...
        check_eb_chunk_change(ox:number,oy:number,x:number,y:number){
            ...
            if(ock!=ck){
                const ck_=this.calc_chunk_pos_(x,y)
                let changed=false
                if(ck_.cx>this.chunk_max_x){
                    this.chunk_max_x=ck_.cx
                    changed=true
                }else if(ck_.cx<this.chunk_min_x){
                    this.chunk_min_x=ck_.cx
                    changed=true
                }
                if(ck_.cy>this.chunk_max_y){
                    this.chunk_max_y=ck_.cy
                    changed=true
                }else if(ck_.cy<this.chunk_min_y){
                    this.chunk_min_y=ck_.cy
                    changed=true
                }
                if(changed){
...
return this
        ...
        set_chunk_range_by_notehandle(notehandle:note.NoteHandle){
            this.chunk_max_x=notehandle.content_data.chunkmaxx
            this.chunk_max_y=notehandle.content_data.chunkmaxy
            this.chunk_min_x=notehandle.content_data.chunkminx
            this.chunk_min_y=notehandle.content_data.chunkminy
            this.update_canvas_chunkpadding()
        }
        ...
        private change_padding(u:number,
                       d:number,
                       r:number,
                       l:number){
...
// console.log("locate_editor_bar",editor_bar_id)
    ...
    export class ContentManager{//由canvas持有
        ...
        canvas_unmount(){
            if(this.canvas.get_content_manager().is_holding_note()){
                AppFuncTs.appctx.unhold_notehandle(
                    this.canvas.get_content_manager().notehandle,this)
            }
            this.chunkloader?.unmount()
        }
        //相关组件都需要跟着变动
        first_load_set(notehandle:note.NoteHandle){
            ...
            if(this.is_holding_note()){
                AppFuncTs.appctx.unhold_notehandle(this.notehandle,this)
            }
            AppFuncTs.appctx.hold_notehandle(notehandle,this)
            ...
            // }
            this.reset(canvas)
            this.chunkhelper.set_chunk_range_by_notehandle(notehandle)
    ...
    export class NoteCanvasDataReacher{
        ...
        get_paths_proxy_man():CanvasPathsProxyMan{
            return new CanvasPathsProxyMan(this)
        }
