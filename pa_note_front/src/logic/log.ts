...
import { AddPathReq, DeleteBarReq, RemovePathReq, UpdateBarContentReq, UpdateBarTransformReq, apis } from "./gen_api_content";
    ...
    export namespace SubTrans {
        ...
        export class EbTrans implements ITrans {
            ...
            redo(handle: note.NoteHandle, log: NoteLogger, ctx: AppFuncTs.Context): void {
                ...
                apis.update_bar_transform(new UpdateBarTransformReq(
        ...
        export class EbEdit implements ITrans {
            ...
            redo(handle: note.NoteHandle, log: NoteLog.NoteLogger, ctx: AppFuncTs.Context): void {
                ...
                apis.update_bar_content(new UpdateBarContentReq(
                    handle.note_id, this.ebid, this.to, this.to
                ))
        ...
        export class EbDel implements ITrans {
            ...
            redo(handle: note.NoteHandle, log: NoteLogger, ctx: AppFuncTs.Context): void {
                ...
                apis.delete_bar(new DeleteBarReq(handle.note_id, this.ebid)).then(_r => {
                    let r = _r.succ()
                    if (!r) return
                    handle.noteloader.set_chunkrange(r.chunk_minx, r.chunk_maxx, r.chunk_miny, r.chunk_maxy, true)
                    // @ts-ignore
        ...
        export class EbConn implements ITrans {
            ...
            redo(handle: note.NoteHandle, log: NoteLogger, ctx: AppFuncTs.Context): void {
                this.conn_pairs.forEach(([ebid1, ebid2]) => {
                    ...
                    apis.add_path(new AddPathReq(
                        handle.note_id, ebid1, ebid2
                    )).then(_r => {
                        // let succ = _r.succ()
                        let fail = _r.fail()
                        if (fail) {
                            console.error("fail add path")
                        }
                    })
            ...
            redo(handle: note.NoteHandle, log: NoteLogger, ctx: AppFuncTs.Context): void {
                this.pathkeys.forEach((v) => {
                    ...
                    if (path) {
                        apis.remove_path(
                            new RemovePathReq(
                                handle.note_id, path.b_bar + "_" + path.e_bar
                            )
                        ).then(_r => {
                            let r = _r.succ()
                            if (!r) return
