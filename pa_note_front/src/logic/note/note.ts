...
import { GetChunkNoteIdsReq, GetNoteBarInfoReq, apis } from "../gen_api_content";
...
export namespace note {
    ...
    export class NoteChunk {
        ...
        load(handle: NoteHandle) {
            apis.get_chunk_note_ids(
                new GetChunkNoteIdsReq(
                ...
                this.noteids.forEach((v) => {
                    apis.get_note_bar_info(
                        new GetNoteBarInfoReq(
                            handle.note_id, v
                        )
                    ).then((_r) => {
                        let r = _r.exist()
        ...
        tick_refresh_expire_bars() {
            while (true) {
                ...
                if (pop) {
                    ...
                    apis.get_note_bar_info(
                        new GetNoteBarInfoReq(this.handle.note_id, pop)
                    ).then(_res => {
                        let res = _res.exist()
                        if (!res) {
                            return
