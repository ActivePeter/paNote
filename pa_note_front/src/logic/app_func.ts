...
import { EventCenter } from "@/logic/event_center";
import { _route } from "@/logic/route";
import { GetNoteMataReq, apis as content_apis } from "./gen_api_content";
...
export module AppFuncTs {
    ...
    export class Context {
        ...
        start_open_note(noteid: string) {
            ...
            //1.获取笔记元信息
            content_apis.get_note_mata(
                new GetNoteMataReq(noteid)
            ).then(_reply => {
                let reply = _reply.succ()
                if (reply) {
                    console.log("get meta", reply)
                    //2.notecanvas根据视野情况加载
                    let handle = note.NoteHandle.create(
                        noteid, note.NoteContentData.get_default());
                    handle.noteloader.set_chunkrange(reply.min_chunkx, reply.max_chunkx, reply.min_chunky, reply.max_chunky)
                    this.ui_refs().main_canvasproxy().get_content_manager().first_load_set(
                        handle
                    );
                    _route.update_route_when_open_note(handle.note_id)
                }
            })
        }
